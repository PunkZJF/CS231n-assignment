...
"""
弄了一个早上，大致说一下怎么配置的把上面那个过程搞定的
首先在cmd中直接进入cs231n目录运行'python setup.py build_ext --inplace'
报错：没有Cpython.Build
原因：cmd的编译环境没有设置成Aconda2
设置成aconda2后再次运行'python setup.py build_ext --inplace'
报错：找不到'vcvarsall.bat'
解决：安装Microsoft Visual C++ Compiler for Python 2.7
