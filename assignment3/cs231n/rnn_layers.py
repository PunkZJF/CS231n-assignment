...
def rnn_step_backward(dnext_h, cache):
  ...
  b = cache['b']
  next_h = cache['next_h']
  dback_tanh = dnext_h * (1 - next_h * next_h)
  dx = dback_tanh.dot(Wx.T)
  dprev_h = dback_tanh.dot(Wh.T)
  dWx = x.T.dot(dback_tanh)
  dWh = prev_h.T.dot(dback_tanh)
  db = np.sum(dback_tanh, axis=0)  
  ##############################################################################
  #                               END OF YOUR CODE                             #
  ##############################################################################
...
def rnn_backward(dh, cache):
  ...
  for t in range(T-1, -1, -1):
     dout = dh[:, t, :] + dprev
     dx[:, t, :], dprev, dWx_loc, dWh_loc, db_loc = rnn_step_backward(dout,cache[t])
     dWx += dWx_loc
     dWh += dWh_loc
     db += db_loc
...
def lstm_backward(dh, cache):
  ...
  #############################################################################
  N, T, H = dh.shape
  x, prev_h, prev_c, Wx, Wh, b, i, f, o, g, next_h, next_c = cache[T-1]
  ...
  for t in range(T-1, -1, -1):
    dout = dh[:, t, :] + dprev_h
    dx[:, t, :], dprev_h, dprev_c, dWx_loc, dWh_loc, db_loc = lstm_step_backward(dout, dprev_c, cache[t]) 
    dWx += dWx_loc
    dWh += dWh_loc
    db += db_loc
